...
use mlx_rs::ops::{PadMode, as_strided, concatenate_axis, maximum, pad, square};
...
impl MelSpectrogram {
    ...
    pub fn forward(&self, audio: &Array) -> Result<Array> {
        ...
        let aligned_length =
            ((original_length + HOP_SIZE as i32 - 1) / HOP_SIZE as i32) * HOP_SIZE as i32;
        let aligned = if aligned_length == original_length {
            audio.clone()
        } else {
            pad(
                audio,
                &[(0, 0), (0, aligned_length - original_length)],
                Array::from(0.0f32),
                Some(PadMode::Constant),
            )?
        };

        let length = aligned.shape()[1];
        let left = aligned
            .index((.., 1..PAD + 1))
            .index((.., (..).stride_by(-1)));
        let right = aligned
            ...
            .index((.., (..).stride_by(-1)));
        let padded = concatenate_axis(&[&left, &aligned, &right], 1)?;
